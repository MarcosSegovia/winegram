...
final class RedisInstagramWriter implements InstagramWriterInterface
{
    ...
    {
        $this->redis_client->rpush(self::LIST_OF_INSTAGRAM_POSTS, [$a_new_post->associatedData()]);
    }

    public function tagPostAsRead(Post $a_new_post)
    {
        $this->redis_client->set(self::UNIQUE_ID_PREFIX . $a_new_post->id(), $a_new_post->id(), self::ONE_WEEK_IN_SECONDS);
    }
}
